...
/*bot.onText(/\/admin/, (msg) => {
  ...
  if (chatId.toString() === adminChatId) {
    // Извлекаем все записи о бронированиях
    dbBook.all('SELECT * FROM bookings', [], (err, rows) => {
      ...
      if (rows.length === 0) {
        bot.sendMessage(chatId, 'Нет записей на экскурсии.');
        return;
      }
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Записи на экскурсию');
      ...
      const filePath = path.join(dir, 'tour_bookings.xlsx');
    ...
    // Если это не администратор, отправляется сообщение
